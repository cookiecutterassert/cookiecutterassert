...
class FileDoesNotContainLineRule:
    ...
    def execute(self, outputFolder):
        if (not path_exists_case_sensitive(self.fileName, parent_path=outputFolder)):
            messager.printError("assertion fileDoesNotContainLine {0} {1} failed. {0} does not exist in {2}.".format(self.fileName, self.line, outputFolder))
            return False
        
        fileLines = readLinesFromFile(self.fileName, folder=outputFolder)
        success = self.line not in fileLines
        if not success:
            messager.printError("assertion fileDoesNotContainLine {0} {1} failed.  Matching line found in {2}/{0}.".format(self.fileName, self.line, outputFolder))
        return success
